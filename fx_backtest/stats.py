...
from fx_lib.models.trade import Trade
...
class Stats:
    ...
    def drawdown_duration(self):
        iloc = np.unique(np.r_[(self.drawdown == 0).values.nonzero()[0], len(self.drawdown) - 1])
        ...
        return df['iloc'].map(self.drawdown.index.__getitem__) - df['prev'].map(self.drawdown.index.__getitem__)
    ...
    def drawdown_peaks(self):
        ...
        # df = df.reindex(self.drawdown.index)
        return df.apply(lambda row: self.drawdown.iloc[row['prev']:row['iloc'] + 1].max(), axis=1)
